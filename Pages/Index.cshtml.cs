...
namespace videoscriptAI.Pages
{
    public class IndexModel : PageModel
    {
        ...
        // Nuove proprietà per gestire il link dinamico all'ultima chat
        ...
        {
            // Inizializza le nuove proprietà per il link all'ultima chat
            LastChatId = null;
            HasChats = false;
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (user != null)
                {
                    try
                    {
                        // Trova l'ultima chat dell'utente
                        var lastChat = await _dbContext.Chats
                        ...
                        if (lastChat != null)
                        {
                            ...
                            HasChats = true;
                            _logger.LogInformation($"Trovata ultima chat con ID {lastChat.Id} per l'utente {user.Id}");
                        }
                        else
                        {
                            _logger.LogInformation($"Nessuna chat trovata per l'utente {user.Id}");
                        }
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError(ex, "Errore durante il recupero dell'ultima chat");
                    }
                }
            }
        ...
        public async Task<IActionResult> OnPostUploadVideoAsync(IFormFile videoFile)
        {
            ...
            try
            {
                ...
                var chat = new videoscriptAI.Data.Chat
                {
                    ...
                    IsAutoGenerated = false // Imposta esplicitamente a false
                };
